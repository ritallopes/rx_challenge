...
async def create_equipment_data(
    data: EquipmentDataCreate, db: Session = Depends(get_db)
):
    equipment = (
        db.query(Equipment).filter(Equipment.equipment_id == data.equipment_id).first()
    )
    if not equipment:
        raise HTTPException(
            status_code=HTTPStatus.NOT_FOUND, detail='Equipment not found'
...
async def update_equipment_data(
    id: int, data_update: EquipmentDataUpdate, db: Session = Depends(get_db)
):
    db_equipment_data = db.query(EquipmentData).filter(EquipmentData.id == id).first()
    if not db_equipment_data:
        raise HTTPException(
            status_code=HTTPStatus.NOT_FOUND, detail='Equipment data not found'
        )
