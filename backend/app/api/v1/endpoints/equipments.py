...
from app.api.db.models import Equipment
...
@router.get('/{equipment_id}', status_code=HTTPStatus.OK, response_model=EquipmentWithDataResponse)
async def read_equipment(equipment_id: str, db: Session = Depends(get_db)):
    ...
    equipment_data = db.query(EquipmentData).filter(EquipmentData.equipment_id == equipment_id).all()
...
async def create_equipment(equipment: EquipmentCreate, db: Session = Depends(get_db)):
    existing_equipment = db.query(Equipment).filter(Equipment.equipment_id == equipment.equipment_id).first()
    if existing_equipment:
        raise HTTPException(status_code=HTTPStatus.CONFLICT, detail='Equipamento já registrado')
...
@router.put('/{equipment_id}', status_code=HTTPStatus.OK, response_model=EquipmentResponse)
async def update_equipment(equipment_id: str, equipment_update: EquipmentUpdate, db: Session = Depends(get_db)):
    db_equipment = db.query(Equipment).filter(Equipment.equipment_id == equipment_id).first()
    if not db_equipment:
        raise HTTPException(status_code=HTTPStatus.NOT_FOUND, detail='Equipamento não encontrado')
